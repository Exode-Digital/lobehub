...
// Default state mocks for the [single-human thread relaxation] participant tracking. Tests that
...
// keep their before the fix behaviour. Individual tests can replace this via
...
describe('BotMessageRouter', () => {
 ...
 // [single-human thread relaxation] participant tracking — restore defaults wiped by
 ...
 // [single-human thread relaxation] introduced single-human thread relaxation: a non-mention
 ...
 it('should wake on a watch-keyword match in a subscribed group thread ', async () => {
    ...
    it('should not change behaviour when watchKeywords is empty/missing', async () => {
      // Sanity: existing call sites must keep their before the fix semantics.
