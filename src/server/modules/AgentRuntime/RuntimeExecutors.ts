...
export const createRuntimeExecutors = (
 ...
 // Parent existence preflight ([fail-fast on tool/assistant persist failures] / [parent-id FK violation during long agent sessions]):
      ...
      for (let attempt = 1; attempt <= maxAttempts; attempt++) {
        ...
        try {
          ...
          const response = await modelRuntime.chat(chatPayload, {
            callback: {
              ...
              onToolsCalling: async ({ toolsCalling: raw }) => {
                ...
                // replaying history.                 const payload = resolvedCalls.map((p) => ({
                  ...p,
          ...
          try {
            ...
            // builds and 400 strict providers like NVIDIA NIM.             const persistedTools =
 ...
 // either duplicate the tool_call_id or violate parent_id FK .
      ...
      // running the agent on a broken conversation chain.       if (isPersistFatal(error)) throw error;
      ...
      toolsToExecute.map(async (chatToolPayload: ChatToolPayload) => {
        ...
        try {
          ...
          try {
            ...
            // the ConversationParentMissing throw is consumed.             const fatal = isParentMessageMissingError(error)
    ...
    if (skipCreateToolMessage) {
      ...
      for (const toolPayload of pendingToolsCalling) {
        ...
        try {
          ...
          // two entries for the same tool_call_id — review P2.
  ...
  resolve_aborted_tools: async (instruction, state) => {
    ...
    for (const toolPayload of toolsCalling) {
      ...
      try {
        ...
        // error instead of the raw driver text .
        const fatal = isParentMessageMissingError(error)
          ? createConversationParentMissingError(parentMessageId, error)
          : error instanceof Error
            ? error
