...
/**
 * Subscribe to Electron IPC broadcasts. As of [Cloud Claude Code V3] phase 0, the main
 ...
 * Required for the **toolless middle step** case : when a step
  ...
  try {
    ...
    /**
 * Process a single `AgentStreamEvent` from main. As of [Cloud Claude Code V3] phase 0,
 ...
 // Phase 1 lives in metadata; Phase 2 promotes to a
        ...
        persistQueue = persistQueue.then(async () => {
          ...
          // in the run (rescues toolless middle steps — ), then
 ...
 // [Cloud Claude Code V3] phase 0; pull it back through the existing `getSessionInfo`
