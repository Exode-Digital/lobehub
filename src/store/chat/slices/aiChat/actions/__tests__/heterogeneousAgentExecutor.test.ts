...
describe('heterogeneousAgentExecutor DB persistence', () => {
 ...
 // Regression for [CC Skill → ToolSearch → MCP chain regression]-style bug: under partial-messages mode, CC
  ...
  // ────────────────────────────────────────────────────
 // Real trace regression: multi-tool per turn ([multi-tool per turn orphan trace] scenario)
  // ────────────────────────────────────────────────────

  describe('multi-tool per turn (real trace regression)', () => {
    /**
 * Reproduces the exact CC event pattern from the [multi-tool per turn orphan trace] orphan trace.
    ...
    it('should register ALL tools on correct assistant when turn has text + multiple tool_uses', async () => {
      ...
      await runWithEvents([
        ...
        ccToolUse('msg_03', 'toolu_getissue', 'mcp__linear__get_issue', { id: 'multi-tool per turn orphan trace' }),
  ...
  // ────────────────────────────────────────────────────
 // [CC Skill → ToolSearch → MCP chain regression] reproduction: Skill → ToolSearch → MCP tool
  //
  ...
  describe('CC Skill → ToolSearch → MCP chain regression Skill → ToolSearch → MCP repro', () => {
    it('persists tool_result content for Skill, ToolSearch, and the deferred MCP tool', async () => {
      ...
      await runWithEvents([
        ...
        ccToolUse('msg_03', 'toolu_get_issue', 'mcp__linear-server__get_issue', {
          id: 'CC Skill → ToolSearch → MCP chain regression',
        }),
  ...
  // ────────────────────────────────────────────────────
 // CC subagent thread-container model ([CC subagent thread-container model] / [CC subagent thread-container model])
  //
  ...
  describe('CC subagent thread-container', () => {
    ...
    /**
 * Regression for [subagent forwarding guard]: the subagent forwarding guard initially only
  ...
  describe('CC Monitor parentId chain regression Monitor parentId chain', () => {
    /**
    ...
    /**
 * [toolless middle step parentId chain fix] regression: a toolless step in the middle must NOT break the
    ...
    /**
 * [toolless middle step parentId chain fix] follow-up: N consecutive toolless steps (Monitor pushing
  ...
  describe('external signal callbacks external signal (metadata.signal)', () => {
 ...
 // Step 4 post-task summary: tagged with `task-completion`       // so MessageCollector renders it inside the same AssistantGroup,
