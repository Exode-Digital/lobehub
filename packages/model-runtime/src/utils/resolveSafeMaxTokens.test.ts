...
describe('assertContextWithinWindow', () => {
  ...
  it('attaches fail-fast when input exceeds context window structured payload via toPayload()', () => {
    const longContent = 'a'.repeat(20_000);
    try {
      assertContextWithinWindow(
        { messages: [{ content: longContent, role: 'user' }], model: 'tight-model' } as any,
 ...
 // Regression test for [fail-fast when input exceeds context window] PR review feedback: the helper was
