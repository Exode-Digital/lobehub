...
export class MessageTranslateService extends BaseService {
  ...
  async getTranslateByMessageId(messageId: string): ServiceResult<MessageTranslateResponse | null> {
    ...
    this.log('info', 'Fetch translation by message ID', { messageId, userId: this.userId });
    ...
    try {
      ...
      this.log('info', 'Translation info retrieved', { messageId });
      return response;
    } catch (error) {
      this.handleServiceError(error, 'Fetch translation by message ID');
    }
  }
  ...
  async translateMessage(
    ...
    this.log('info', 'Start translating message', {
      ...translateData,
      userId: this.userId,
    });
  ...
  async updateTranslateInfo(
    ...
    try {
      ...
      if (!messageInfo) {
        throw this.createCommonError('Message for translation update not found');
      }
      ...
      this.log('info', 'Translation info updated', { messageId: data.messageId });
  ...
  async deleteTranslateByMessageId(
    ...
    try {
      ...
      if (!originalTranslation) {
        throw this.createNotFoundError('Translation does not exist');
      }
