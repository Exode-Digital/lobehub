...
describe('GeneralChatAgent', () => {
 ...
 // [token counter compression budget] Bug B: state.tools must feed into the compression budget,
 ...
 // Regression for [recover malformed tool_call names]: when the LLM emits tool_calls whose names
 ...
 // [token counter compression budget] follow-up: when state.forceFinish is set, RuntimeExecutors strips
